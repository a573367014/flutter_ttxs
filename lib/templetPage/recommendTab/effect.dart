...
Effect<RecommendTabState> buildEffect() {
  return combineEffects(<Object, Effect<RecommendTabState>>{
    ...
    RecommendTabActionEnum.onLoadTemplate: _onLoadTemplate
  });
}
...
void _loadData(Action action, Context<RecommendTabState> ctx) {
  Future.wait([
    ...
    ctx.dispatch(IndexPageActionCreator.onLoadTemplate(
        list.sublist(0, min(list.length, 20))));
  }).catchError((e) {
    print('_loadData:' + e);
  });
}

Future<List<TemplateListState>> _onLoadTemplate(
  ...
  final List<List<KeywordState>> chunks = chunk<KeywordState>(list, 5);
  ...
  return Future.forEach(chunks, (List<KeywordState> kwList) {
    ...
    ctx.dispatch(IndexPageActionCreator.updateTemplateLists(result));
    return result;
  }).catchError((e) {
